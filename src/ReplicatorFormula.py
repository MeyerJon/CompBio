...
class Replicator:
    ...
    def calculate_one_step(self, populations):
        ...
        for group in self.populations[0].groups.keys():
            new_size = self.populations[0].groups[group] * pop1_group_fitnesses[group] / pop1_avg_fitness
            self.populations[0].groups[group] += (new_size - self.populations[0].groups[group]) * config.timestep
        ...
        for group in self.populations[1].groups.keys():
            new_size = self.populations[1].groups[group] * pop2_group_fitnesses[group] / pop2_avg_fitness
            self.populations[1].groups[group] += (new_size - self.populations[1].groups[group]) * config.timestep
